...
//nestedFunction is in the scope of myFunction, so because of closure allowing functions inside the scope (inside the {}) of other functions to access its variables it can.
...
function animalNames(zooArr) {
	const displayNames = [];
	zooArr.forEach(function (aniName) {
		displayNames.push(
			`name: ${aniName.animal_name}, scientific: ${aniName.scientific_name}`
		);
	});
	// console.log(`aniname` + displayNames);
	return displayNames;
}
...
function lowerCaseNames(zooArr) {
	const CaseName = [];
	zooArr.map(function (aniName) {
		CaseName.push(`${aniName.animal_name}`.toLowerCase());
	});

	// console.log(`low case +++++++>     `, CaseName);
	return CaseName;
}
...
function lowPopulationAnimals(zooArr) {
	const lowPop = zooArr.filter(function (zooArr) {
		return zooArr.population < 5;
	});
	// console.log("lowpop ====>   ", lowPop);
	return lowPop;
}
...
function USApop(zooArr) {
	const popArr = [];
	zooArr.map(function (zooPop) {
		popArr.push(zooPop.population);
		// console.log(`poparr===>   `, popArr);
	});
	const pop = popArr.reduce(function (initial, accum) {
